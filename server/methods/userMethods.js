...
// 简单的加密函数 - 生产环境应使用更安全的加密方法
function encryptApiKey(apiKey, userId) {
  // 这里仅作为示例，实际应用中应使用更安全的加密方法
  const encoder = new TextEncoder();
  const data = encoder.encode(apiKey + userId);
  return Buffer.from(data).toString('base64');
}

// 解密函数
function decryptApiKey(encryptedKey, userId) {
  // 对应上面的加密方法的解密实现
  const decoded = Buffer.from(encryptedKey, 'base64');
  const decoder = new TextDecoder();
  const decrypted = decoder.decode(decoded);
  return decrypted.slice(0, -userId.length); // 移除添加的userId
}
...
Meteor.methods({
  ...
  // 更新用户通用设置
  'users.updateSettings'(settings) {
    if (!this.userId) {
      throw new Meteor.Error('not-authorized', '用户未登录');
    }
    
    check(settings, {
      theme: Match.Maybe(String),
      language: Match.Maybe(String),
      timezone: Match.Maybe(String),
      dateFormat: Match.Maybe(String),
      notifications: Match.Maybe({
        enabled: Boolean,
        sound: Boolean,
        desktop: Boolean
      })
