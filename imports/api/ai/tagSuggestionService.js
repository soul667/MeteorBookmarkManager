...
// OpenAI客户端将在每次请求时动态创建，使用用户的API密钥
async function createOpenAIClient() {
  const user = Meteor.users.findOne(Meteor.userId());
  if (!user || !user.profile?.aiSettings?.apiKey) {
    throw new Meteor.Error('api-key-required', '需要配置OpenAI API密钥');
  }
  
  return new OpenAI({
    apiKey: user.profile.aiSettings.apiKey,
  });
}
