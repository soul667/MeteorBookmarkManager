...
export const ClassificationService = {
  ...
  async suggestFolder(content, userId) {
    try {
      ...
      if (!userFolders || userFolders.length === 0) {
        return await this.suggestNewFolder(content);
      }
      ...
      if (result.includes('需要新文件夹')) {
        ...
        if (match) {
          ...
          const folderSuggestion = await this.suggestNewFolder(content);
  ...
  // 建议新文件夹名称
  async suggestNewFolder(content) {
    try {
      // 构建内容的简洁描述
      const contentSummary = `
        标题: ${content.title || ''}
...
Meteor.methods({
  ...
  async 'ai.suggestNewFolder'(content) {
    ...
    return await ClassificationService.suggestNewFolder(content);
  }
});